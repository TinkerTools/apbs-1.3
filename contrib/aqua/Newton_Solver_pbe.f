...
c        (NEwton-based PDE solver for PB equations, based
c        on Michael Holst MG package)
c
c        Version 1:      7/4/07
c
c        Author: Patrice Koehl (in collaboration with Marc Delarue)
c
c        This file contains a series of subroutines for solving
c        modified Poisson-Boltzmann equations. It is inspired
c        form Michael Holst MG code, which is available at:
...
c ==========================================================================================
c
c        Newton_Solver implements the inexact Newton Multilevel
c        solver applied by Michael Holst for solving the
c        Poisson-Boltzmann equation, and currently implemented
c        in APBS.
c
...
c
        ...
        real*8  bndx(*),bndy(*),bndz(*)
        real*8  eps_X(*),eps_Y(*),eps_Z(*),helm(*),rhs(*)
c
c        Local variables:
c ==========================================================================================
c
        integer i,nx,ny,nz,niwk,nrwk,nlevel,ierror
        integer maxlev,mxlevel,mgcoar,mgdisc,mgsolv
...
c
        if(nlevel.le.0.or.nx.le.0.or.ny.le.0.or.nz.le.0) then
           write(6,*)
     &        'nx,ny,nz and nlevel must be strictly positive!!'
           ierror = -1
           iparam(51) = ierror
           return
        endif
c
c        Check that the number of levels proposed
c        is compatible with the grid size
c
...
c
c ==========================================================================================
c        Get grid size info for all grid levels: this will be used
c        to partition the arrays iwork and rwork so that they
c        can store the whole hierarchy of grids
c ==========================================================================================
c
        call mgszaqua(mgcoar,mgdisc,mgsolv,nx,ny,nz,nlevel,
     1                nxc,nyc,nzc,nf,nc,narr,narrc,n_rpc,
     2                n_iz,n_ipc,ireal_tot,iint_tot)
c
...
c
        if(nrwk.lt.ireal_tot) then
           write(6,*) 'Real work space mut be at least: ',
     &                ireal_tot
           ierror = -3
           iparam(51) = ierror
           return
...
c
     ...
     &                rwork(k_w1),rwork(k_w2),iwork(k_ipc),
     &                rwork(k_rpc),rwork(k_ac),rwork(k_cc),
     &                rwork(k_fc),xpos,ypos,zpos,bndx,bndy,bndz,
     &                eps_X,eps_Y,eps_Z,helm,rhs)
c
c ==========================================================================================
...
c ==========================================================================================
c
c        This subroutine uses a Newton method, combined with
c        a linear multigrid solver, to solve modified
c        Poisson-Boltzmann equations.
c
        subroutine newton_mg_pbe(iparam,rparam,nx,ny,nz,u,iz,
     1                w1,w2,ipc,rpc,ac,cc,fc,xpos,ypos,zpos,
...
c ==========================================================================================
c
        integer mgkey,nlevel,itmax,iok
        integer iinfo,istop,ipkey,nu1,nu2
        integer nx,ny,nz,ilev,ido,iters
        integer ierror,nlevel_real
...
c
        ido = 0
        call build_operator(nx,ny,nz,nlevel,ipkey,iinfo,ido,
     &                      iz,mgprol,mgcoar,mgsolv,mgdisc,ipc,
     &                      rpc,ac,cc,fc,xpos,ypos,zpos,gxcf,
...
c
        if(mgkey.eq.0) then
           call newton_vcycle_pbe (nx,ny,nz,u,iz,ccf,fcf,w1,w2,
     1                        istop,itmax,iters,ierror,nlevel,ilev,
     2                        nlevel_real,mgsolv,iok,iinfo,epsilon,
     3                        errtol,omegan,nu1,nu2,mgsmoo,a1cf,
     4                        a2cf,a3cf,ipc,rpc,ac,cc,fc)
        elseif(mgkey.eq.1) then
           call newton_nested_pbe (nx,ny,nz,u,iz,ccf,fcf,w1,w2,
     1                        istop,itmax,iters,ierror,nlevel,ilev,
     2                        nlevel_real,mgsolv,iok,iinfo,epsilon,
     3                        errtol,omegan,nu1,nu2,mgsmoo,a1cf,
     4                        a2cf,a3cf,ipc,rpc,ac,cc,fc)
...
c
        subroutine newton_nested_pbe(nx,ny,nz,u,iz,w0,w1,w2,w3,
     1                        istop,itmax,iters,ierror,nlevel,ilev,
     2                        nlevel_real,mgsolv,iok,iinfo,epsilon,
     3                        errtol,omega,nu1,nu2,mgsmoo,cprime,
     3                        rhs,utemp,ipc,rpc,ac,cc,fc)
c
c ==========================================================================================
c         Declare all variables
c ==========================================================================================
...
c
        integer nx,ny,nz
        integer istop,itmax,iters,ierror
        integer level,nlevel,ilev,nlevel_real
...
c               errd = errtol
                ...
                iokd = iok
                istpd = istop
                call newton_vcycle_pbe(nxc,nyc,nzc,u,iz,w0,w1,
     1                w2,w3,istpd,itmxd,iterd,ierror,nleveld,
     2                level,nlevel_real,mgsolv,iokd,iinfo,
     3                epsilon,errd,omega,nu1,nu2,mgsmoo,
     3                cprime,rhs,utemp,ipc,rpc,ac,cc,fc)
c
                call make_fine(1,nxc,nyc,nzc,nxf,nyf,nzf)
c
                call interpolate(nxc,nyc,nzc,nxf,nyf,nzf,
     1                u(iz(1,level)),u(iz(1,level-1)))
c
                nxc = nxf
                nyc = nyf
                nzc = nzf
...
c
        level = ilev
        call newton_vcycle_pbe(nxc,nyc,nzc,u,iz,w0,w1,w2,
     1          w3,istop,itmax,iters,ierror,nlevel,level,
     2          nlevel_real,mgsolv,iok,iinfo,epsilon,
     &          errtol,omega,nu1,nu2,mgsmoo,cprime,
     3          rhs,utemp,ipc,rpc,ac,cc,fc)
c
        return
        end
...
c
        subroutine newton_vcycle_pbe(nx,ny,nz,x,iz,w0,w1,w2,w3,
     1                        istop,itmax,iters,ierror,nlev,ilev,
     &                        nlevel_real,mgsolv,iok,iinfo,epsilon,
     2                        errtol,omega,nu1,nu2,mgsmoo,cprime,
     3                        rhs,xtemp,ipc,rpc,ac,cc,fc)
c
c ==========================================================================================
c         Declare all variables
c ==========================================================================================
...
c
        ...
        call residual(nx,ny,nz,ipc(iz(5,lev)),rpc(iz(6,lev)),
...
c
                ipkey = ipc(10)
c
                call get_jacobian(nx,ny,nz,nlevel_real,iz,ilev,
     1                            ipkey,x,w0,cprime,rhs,cc)
c
c
...
c
                ...
                iinfo_s  = iinfo
c
                call lin_mg_vcycle(nx,ny,nz,xtemp,iz,w0,w1,
     1                w2,w3,istop_s,itmax_s,iters_s,ierror_s,
     2                nlev,ilev,nlevel_real,mgsolv,iok_s,
     3                iinfo_s,epsilon,errtol_s,omega,nu1,
     4                nu2,mgsmoo,ipc,rpc,ac,cprime,rhs)
c
...
c
     ...
     1                        ipc(iz(5,lev)),rpc(iz(6,lev)),
     2                        ac(iz(7,lev)),cc(iz(1,lev)),
     3                        fc(iz(1,lev)),w1,w0,rhs(iz(1,lev)))
c
...
c
                    damp = 1.0d0
                    call vectsmult(nx,ny,nz,damp,
     1                             xtemp(iz(1,lev)),x(iz(1,lev)))
c
                    call residual(nx,ny,nz,ipc(iz(5,lev)),
     1                        rpc(iz(6,lev)),ac(iz(7,lev)),
     2                        cc(iz(1,lev)),fc(iz(1,lev)),
...
c
                if(iok.ne.0) then
                        ...
                        call print_residual(iok,iters,rsnrm,
     1                                      rsden,orsnrm)
...
c
        subroutine get_jacobian(nx,ny,nz,nlev_real,iz,lev,
     1                          ipkey,x,r,cprime,rhs,cc)
c
c ==========================================================================================
c         Declare all variables
c ==========================================================================================
...
c
        call dc_vec(cc(iz(1,lev)),x(iz(1,lev)),
     1              cprime(iz(1,lev)),nx,ny,nz,ipkey)
c
